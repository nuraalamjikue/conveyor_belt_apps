...
interface EmpData {
  ...
  membertype: number;
  payrollid: string;
  payrollconnectid: string; // Added this property to match the usage
  mobilenumber: string;
}

const EmployeeCardWiseOnlineCigaretteSale: React.FC = () => {
  ...
  const handleSubmitData = async () => {
    ...
    // console.log(
    //   'Checking if empmembershipdata ' + JSON.stringify(empData?.id, null, 2),
    // );
    ...
    const SaveData = selectedValue.map(item => ({
      ...
      payrollConnectionId: empData?.payrollconnectid,
    ...
    try {
      ...
      //   console.log(
      //     'response.data.res ' + JSON.stringify(response.data.res, null, 2),
      //   );
  ...
  const handelScannFrist = () => {
    handleToastMsg('error', `Scann Frist `);
  };
...
export default EmployeeCardWiseOnlineCigaretteSale;
